...
def resolve_blob_to_local(blob_path: str) -> Path | None:
    ...
    # 3. Azure download -> local cache (atomic write to prevent TOCTOU).
    if _azure_ready():
        try:
            ...
            tmp.replace(local)  # Readers always see complete file.
            logger.info(
            ...
            logger.warning("[blob_resolver] Azure error for %r: %s", blob_path, exc)
